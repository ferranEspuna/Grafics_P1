...
Plane::Plane(vec3 normal, vec3 pass_point){
    this->normal = normalize(normal);
    this->point = pass_point;
}
...
bool Plane::closestHit(Ray &raig, HitInfo &info){
    ...
    //CAMBIADO DE NEG A POSITIVE TODO
    //INTENSIDAD DE COLOR DEPENDA DE LA DISTANCIA.
...
bool Plane::hasHit (const Ray &raig){
    // Comprovem interseccio entre el pla i el raig
