...
void PropertyInfo::read(const QJsonObject &json)
{
    ...
    if (json.contains("minValueReal") && json["minValueReal"].isDouble()) {
        minValueReal = json["minValueReal"].toDouble();
    }else{
        if (json.contains("minValue") && json["minValue"].isDouble()) {
            minValueReal = json["minValue"].toDouble();
        }
    }
    if (json.contains("maxValueReal") && json["maxValueReal"].isDouble()) {
        maxValueReal = json["maxValueReal"].toDouble();
    }else{
        if (json.contains("maxValue") && json["maxValue"].isDouble()) {
            maxValueReal = json["maxValue"].toDouble();
        }
    }
...
void PropertyInfo:: write(QJsonObject &json) const
{
    ...
    json["minValueReal"] = minValueReal;
    json["maxValueReal"] = maxValueReal;
...
void PropertyInfo::print(int indentation) const
{
    ...
    QTextStream(stdout) << indent << "minValueReal:\t" << minValueReal << "\n";
    QTextStream(stdout) << indent << "maxValueReal:\t" << maxValueReal << "\n";
