...
Camera::Camera() {
    ...
    computeAtributes(lookfrom, lookat, vup, 180.0, 1.0, viewportX, false, 1.0);
}
...
void Camera::read (const QJsonObject &json)
{
    ...
    if (json.contains("defocusBlur") && json["defocusBlur"].isObject()) {
...
void Camera::print(int indentation) const
{
    ...
    QTextStream(stdout) << indent.toInt()+4 << "enabled:\t" << defocus_blur << "\n";
    QTextStream(stdout) << indent.toInt()+4 << "aperture:\t" << lens_radius*2.0 << "\n";
